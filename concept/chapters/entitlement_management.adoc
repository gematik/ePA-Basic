...
Zum Erstellen einer Befugnis ist ein Anwesenheitsnachweis der eGK verpflichtend, der über zwei Wege geführt werden kann:

* Es wird ein Prüfungsnachweis erzeugt, der aus der Durchführung des VSDM-Anwendungsfalls "ReadVSD" im Konnektor/TI-Gateway resultiert. Damit der Prüfungsnachweis in Verbindung zur Umgebung gesetzt werden kann, wird dieser zudem mit der C.HCI.AUT-Identität der SMC-B signiert, bevor er im *Entitlement Management* registriert wird.
* Es wird das *Proof of Patient Presence (PoPP)* der TI genutzt. Dabei wird über den lokalen PoPP-Client die Erzeugung eines PoPP-Token am PoPP Service ausgelöst und dieses PoPP-Token dann am *Entitlement Management* registriert. Ein PoPP-Token attestiert die Anwesenheit einer eGK in einer bestimmten Umgebung.

_Hinweis: Die Möglichkeit der Verwendung eines VSDM-Prüfnachweises wird zukünftig entfallen. Daher ist er in der Abbildung nicht mehr dargestellt._
