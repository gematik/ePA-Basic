...
ifdef::backend-pdf[]
[.metadata]
//Version: 3.1.2 Release, tt.mm.2025
endif::[]
...
|11.07.2024
|4
|Anpassung Service-Lokalisierung; Veröffentlichung
|gematik
...
|15.07.2024
|
|Veröffentlichung zur Kommentierung
|gematik
...
|28.05.2025
|
|Ergänzung der Deny-Liste zum Ausschluss von Telematik-Ids; Veröffentlichung
|gematik

|3.1.2-1
|15.07.2025
|
|Ergänzung der XML- und JSON-Dokumente in den MHD Volltextindex; Vorveröffentlichung
|gematik
...
Die *Vertrauensstelle des RKI* (VST) ermöglicht es die verschiedenen Datenfreigaben an das *FDZ* durch einen Versicherten zusammenzuführen, ohne den Versicherten identifizierbar zu machen. Die *VST* übersetzt die Lieferpseudonyme in ein periodenübergreifendes Pseudonym.
...
Das *Forschungsdatenzentrum* (FDZ) ist der Empfänger der pseudonymisierten Daten des Versicherten, die bei fehlendem Widerspruch automatisch an das *FDZ* übermittelt werden. Das *FDZ* übernimmt die übermittelten Daten unter Verwendung des periodenübergreifenen Pseudonyms der VST in sein Datenmodell und bedient daraus Anfragen der Forschung unter Bewahrung der Annonymität des Versicherten. Der Versicherte kann die Verwendung seiner Daten auf bestimmte Sekundärnutzungszwecke einschränken.
...
* den *Secure Admin Storage*, in dem Befugnisse der ePA gespeichert werden.
...
Da in der TI die Möglichkeit besteht, dass eine LEI aufgrund sektoraler Vorgaben z.B. bei einer Praxisauflösung temporär im Besitz einer gültigen SMC-B bleibt und diese zur Authentisierung ggü. dem *IDP-Dienst* nutzen könnte obwohl sie nicht mehr zugriffsberechtigt auf die ePA für alle ist, gibt es eine Möglichkeit, deren Telematik-ID über eine Deny-List vom Zugriff auf die ePA auszuschließen. Mittelfristig soll dies aber anwendungsübergreifend im *IDP-Dienst* geschehen.
...
Die ePA-Clients (Primärsystem, ePA-FdV, E-Rezept-Fachdienst oder auch ein Clientsystem der Kostenträger (KTR) oder der Ombudsstelle) halten den ermittelten zuständigen Service-Endpunkt für eine Akte vor. Sollte diese Information nicht im ePA-Client vorliegen, wendet sich der ePA-Client an den *Information Service* eines Aktensystems, um dort nach der Akte zu fragen und wenn dort für die gegebene Krankenversicherungsnummer (KVNR) eine Akte existiert den Service-Endpunkt lokal zu speichern. Ist keine Akte auf diesem Aktensystem vorhanden, erfolgt die Abfrage am anderen Aktensystem.
...
Ein ePA-Client (Primärsystem, ePA-FdV, Clientsystem der Kostenträger, Ombudsstelle oder NCPeH-Fachdienst) oder der E-Rezept-Fachdienst baut einen Kanal in die VAU des Aktensystems auf und authentifiziert dabei die VAU als authentische VAU des Anbieters. Nachfolgend wird eine User Session für den Nutzer angelegt und der Nutzer mithilfe des *IDP-Dienstes*, des *Sektoralen IdPs (GesundheitsID)* oder über einen mit dem Zertifikatsprofil C.FD.AUT zugehörigen Schlüsssel - selbst signiertes IDToken (nur Dienste der TI) - authentifiziert.
...
=== Statusmodell der Akte

Eine Akte kann in ihrer Lebensphase verschiedene Status durchlaufen. Diese werden nachfolgend dargestellt. In konkreten Produkten können sich noch weitere Status ergeben, die in der Aussensicht aber immer auf einen der genannten Status zurückgeführt werden müssen.

ifndef::env-github[]

[#fig:statemachine]
[plantuml, "{plantumlsimages}/statemachine", svg]
[caption="Abbildung {counter:figure}: ", reftext="Abbildung {figure}", title="Statusmodell einer Akte",width=100%, pdfwidth=100%,float="center",align="center",title-align="center"]
....
include::{plantumlsdir}/statemachine.puml[]
....
...
image::plantuml/statemachine.svg[caption="Abbildung {counter:figure}: ", reftext="Abbildung {figure}", title="Statusmodell einer Akte",width=100%, pdfwidth=100%,float="center",align="center",title-align="center"]
...
include::chapters/data_submission.adoc[]
include::chapters/eu_access.adoc[]
include::chapters/push_notification.adoc[]
endif::[]
ifdef::env-github[]
...
link:./chapters/consent_management.adoc[Widerspruchsmanagement (Consent Management)]
...
link:./chapters/data_submission.adoc[Datenübermittlung an das FDZ (Data Submission Service)]

link:./chapters/eu_access.adoc[EU-Zugriff]

link:./chapters/push_notification.adoc[Push Notification Management]
...
In den folgenden Links oder Kapiteln werden technische Konzepte zu verschiedenen Medical Services der ePA vorgestellt, die als Basis für die Spezifikation dienen.

[#_documentservices]
=== Dokumentenspezifische Services

Die ePA für alle unterstützt eine dokumentenzentrierte Arbeit über zwei Dienste.

==== XDS Document Service

Zentraler Teil dieser Services ist der *XDS Document Service*, der in
ifndef::env-github[]
link:https://github.com/gematik/ePA-XDS-Document/tree/ePA-3.1.2/concept/gemKPT_ePAfuerAlleXdsDoc_V3.1.2.pdf[Dokumentenmanagement (XDS Document Service)]
endif::[]
ifdef::env-github[]
link:https://github.com/gematik/ePA-XDS-Document/tree/ePA-3.1.2/concept/concept.adoc[Dokumentenmanagement (XDS Document Service)]
endif::[]
beschrieben wird.

// MHD Service
ifndef::env-github[]
include::chapters/mhd_service.adoc[]
endif::[]
ifdef::env-github[]
==== MHD Service

Der *MHD Service* bietet Zugriff auf Dokumente, die in der elektronischen Patientenakte gespeichert sind und erlaubt die Volltextsuche in PDF/A-Dokumenten in der ePA für alle. Das entsprechende Konzept ist in
link:./chapters/mhd_service.adoc[MHD Service]
beschrieben
endif::[]
...
Die ePA unterstützt verschiedene Versorgungsprozesse mittels dedizierter Medical Services. Initial unterstützt die Fachanwendung ePA den *digital gestützten Medikationsprozess* (dgMP) über einen FHIR Data Service.

ifndef::env-github[]
link:https://github.com/gematik/ePA-Medication/tree/ePA-3.1.2/concept/gemKPT_ePAfuerAlleMedSvc_V3.1.2.pdf[Medikationsprozess (Medication Service)]
endif::[]
ifdef::env-github[]
link:https://github.com/gematik/ePA-Medication/tree/ePA-3.1.2/concept/concept.adoc[Medikationsprozess (Medication Service)]
endif::[]
...
* FDZ – Forschungsdatenzentrum Gesundheit beim BfArM
...
* VST – Vertrauensstelle Elektronische Patientenakte vom RKI
